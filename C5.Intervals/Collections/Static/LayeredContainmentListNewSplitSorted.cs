...
namespace C5.Intervals
{
    ...
    public class LayeredContainmentListNewSplitSorted<I, T> : CollectionValueBase<I>, IIntervalCollection<I, T>
        ...
        where T : IComparable<T>
    {
        ...
        private readonly int[] _nextLayers;
        ...
        public LayeredContainmentListNewSplitSorted(IEnumerable<I> intervals)
        {
            ...
            constructLayers(out _intervals, out _pointers, out _nextLayers, out _layerCount, out _firstLayerCount);

            // Cache span value
            _span = new IntervalBase<T>(_intervals[0], _intervals[_firstLayerCount - 1]);
        }

        private void constructLayers(out I[] intervals, out int[] pointers, out int[] nextLayers, out int layerCount, out int firstLayerCount)
        {
            // Sort intervals
            Sorting.Timsort(_intervalArray, IntervalExtensions.CreateComparer<I, T>());

            // Pointer is used to store the layers length
            var layers = new ArrayList<Node> {
                // The main layer
                new Node {
                    Interval = _intervalArray[0],
                    Length = 1
                },
            ...
            nodes[0] = new Node { Interval = _intervalArray[0], Layer = 0, Pointer = 0 };

            // Figure out each intervals placement
            for (var i = 1; i < _count; ++i)
            {
                var interval = _intervalArray[i];
                // Find which layer interval must be stored in
            ...
            for (var l = 0; l < layerCount; ++l)
                nodes[_count + l] = new Node { Layer = l, Pointer = layers[l + 1].Length, NextLayer = l + 1 };
            ...
            nextLayers = new int[_count + layerCount];

            for (var i = 0; i < nodes.Length; ++i)
            {
                intervals[i] = nodes[i].Interval;
                pointers[i] = nodes[i].Pointer;
                nextLayers[i] = nodes[i].NextLayer;
            }
        }
        ...
        /// <inheritdoc/>
        public IEnumerable<I> Sorted { get { return _intervalArray; } }
        ...
        public IEnumerable<I> FindOverlapsSorted(T query)
        {
            ...
            var result = findOverlapsMostlySorted(queryInterval).ToArray();
            Sorting.Timsort(result, IntervalExtensions.CreateComparer<I, T>());
            return result;
        }
        ...
        public IEnumerable<I> FindOverlapsSorted(IInterval<T> query)
        {
            ...
            var result = findOverlapsMostlySorted(query).ToArray();
            Sorting.Timsort(result, IntervalExtensions.CreateComparer<I, T>());
            return result;
        }

        private IEnumerable<I> findOverlapsMostlySorted(IInterval<T> query)
        {
