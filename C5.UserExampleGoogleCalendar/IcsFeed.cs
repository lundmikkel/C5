...
namespace C5.UserExampleGoogleCalendar
{
    class IcsFeed
    {
        ...
        public static IEnumerable<CalendarEvent> ParseString(string feed)
        {
            ...
            do
            {
                var low = stringToDate(feed.Substring(index + "DTSTART:".Length, dsLen));
                feed = feed.Substring(index + dsLen + "DTEND:".Length + "DTSTART:".Length + "\r\n".Length);
                var high = stringToDate(feed.Substring(0, dsLen));
                var titleIndex = feed.IndexOf("SUMMARY:", System.StringComparison.Ordinal);
                var title = feed.Substring(titleIndex + "SUMMARY:".Length);
                ...
                index = feed.IndexOf("DTSTART:", StringComparison.Ordinal);
            } while (index >= 0);
